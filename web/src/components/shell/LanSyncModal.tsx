...
export function LanSyncModal({
  ...
  const shareIpText = useMemo(() => {
    ...
    const ips = hosts.filter((h) => h !== "localhost" && h !== "127.0.0.1");
    return ips.length ? ips.join(" / ") : "未获取到";
  }, [serverInfo?.hosts]);
  ...
  const confirmImport = useCallback(async () => {
    ...
    try {
      ...
      setImportResult(`导入成功：新增关注 ${result.addedStreamers}，新增文件夹 ${result.addedFolderCount}，文件夹新增条目 ${result.addedFolderItems}。`);
    } catch (e) {
