...
export type LineOption = { key: string; label: string };

export const lineOptionsByPlatform: Partial<Record<Platform, LineOption[]>> = {
  [Platform.DOUYU]: [
    { key: "ws-h5", label: "主线路" },
...
export const getLineOptionsForPlatform = (platform?: Platform | null): LineOption[] => {
  if (!platform) return [];
  return lineOptionsByPlatform[platform] ?? [];
};

export const resolveStoredLine = (platform?: Platform | null): string | null => {
...
export const persistLinePreference = (platform?: Platform | null, lineKey?: string | null) => {
  if (!platform || !lineKey || typeof window === "undefined") return;
  try {
...
export const resolveCurrentLineFor = (platform: Platform, currentLine: string | null): string | null => {
  const options = getLineOptionsForPlatform(platform);
  if (!options.length) return null;
