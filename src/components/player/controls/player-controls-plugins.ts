...
export class VolumeControl extends Plugin {
  ...
  private onVolumeChange: (() => void) | null = null;
...
export class QualityControl extends Plugin {
  ...
  private openDropdown() {
    if (this.isSwitching || !this.dropdown) return;
    if (this.hoverCloseTimer) {
      clearTimeout(this.hoverCloseTimer);
      this.hoverCloseTimer = null;
    }
...
export class LineControl extends Plugin {
  ...
  private getCurrentKey() {
    return typeof this.config.getCurrentKey === "function" ? this.config.getCurrentKey() : "";
  }

  private getCurrentLabel() {
    return typeof this.config.getCurrentLabel === "function" ? this.config.getCurrentLabel() : "线路";
  }
