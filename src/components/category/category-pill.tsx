...
export function CategoryPill({ label, children, active = false, isDark, size = "md", className, onClick, title }: CategoryPillProps) {
  ...
  const baseClass = "flex items-center gap-2.5 rounded-2xl transition-all duration-300 min-w-max";
  const sizeClass = size === "sm" ? "px-3 py-1.5 text-xs min-w-fit" : "px-4 py-2.5 text-sm";
